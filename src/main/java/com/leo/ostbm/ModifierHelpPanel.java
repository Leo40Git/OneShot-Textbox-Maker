...
public class ModifierHelpPanel extends JPanel {
    ...
    public ModifierHelpPanel() {
        ...
        add(new JLabel(
                "<html>Modifiers are formatted like so:<br /><code>\\m[parameters]</code><br />If a modifier does not need any parameters, the square brackets can be omitted.<br />To add a backslash to the textbox, simply write it twice, like so: <code>\\\\</code></html>"));
        add(new JSeparator());
        add(new JLabel(
